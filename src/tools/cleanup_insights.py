...
# Delete insights related to a specific project
def delete_insights(project_id: int) -> Tuple[int, int]:
    """
    Delete previously generated insights for a project.
    Returns: (deleted_metrics, deleted_files)
    ...
    return deleted_metrics, deleted_files


# Entry point for command-line execution
def main():
    ap = argparse.ArgumentParser(description="Delete previously generated insights for a project.")
    ap.add_argument("--project-id", "-p", type=int, required=True, help="uploaded_files.id / project_id to clean")
    args = ap.parse_args()

    m, f = delete_insights(args.project_id)
    print(f"[CLEANUP] project_id={args.project_id} | project_metrics deleted={m}, file_contents deleted={f}")
